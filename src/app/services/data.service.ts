...
export class DataService {
  ...
  filterItems(searchTerm) {
    return this.dataValue.filter(dataValue => {
      return dataValue.name.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1;
    });
  }

  filterMsg(searchTerm) {
    return this.dataValue.filter(dataValue => {
      return dataValue.message.toLowerCase().indexOf(searchTerm.toLowerCase()) > -1;
    });
  }
